...
export function MultiSelect({ placeholder = "Select options", options, value, onChange, className }: MultiSelectProps) {
  ...
  const filteredOptions = options.filter(
    (option) => option.label.toLowerCase().includes(searchValue.toLowerCase()) && !value.includes(option.value),
  )
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2 p-4 bg-[#0f172a] border border-[#1a222f] rounded-md">
          {value.map((val) => {
            ...
            return (
              <div key={val} className="flex items-center gap-1 px-4 py-2 rounded-full bg-[#1e293b] text-[#f1f6fb]">
